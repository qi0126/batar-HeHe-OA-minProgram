...
Page({
  ...
  //评价
  goDetail(elem){
    // console.log("aaaaakkk", elem.detail.id)
 
    wx.navigateTo({
      url: `/pages/projectAuditDetails/projectAuditDetails?id=${elem.detail.id}`,
    })
    // wx.navigateTo({
    //   url: `/pages/projectAuditDetails/projectAuditDetails`,
    // })
    // console.log('评价11');
    // let self = this;
    // this.setData({
  ...
  //选项卡事件
  tasFun(e) {
    // console.log('pppphx', e.currentTarget.dataset.index);
  ...
  getPlanFun(type) {
    ...
    app.$api.findByAuth(params).then(res => {
      wx.hideLoading()
